...
create_instruction(const ParsedLine &pl,
  ...
  if (op_name == "slice") {
  ...
  else if (op_name == "load_01") {
    // currently only implemented for one attribute: n
    int64 n = 0;
    auto start = attr_str.find("'");
    auto end = attr_str.rfind("'");
    if (start != std::string::npos && end != std::string::npos && end > start) {
      std::string n_str = attr_str.substr(start + 1, end - start - 1);
      try {
        n = std::stoll(n_str);
      } catch (...) {
        return panic("failed to parse n attribute as integer");
      }
    } else {
      return panic("missing n attribute");
    }

    return new Load01(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "load_03") {
    // currently only implemented for one attribute: n
    int64 n = 0;
    auto start = attr_str.find("'");
    auto end = attr_str.rfind("'");
    ...
    return new Load03(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "store_10") {
    // currently only implemented for one attribute: n
    int64 n = 0;
    auto start = attr_str.find("'");
    auto end = attr_str.rfind("'");
    ...
    return new Store10(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "store_30") {
    // currently only implemented for one attribute: n
    int64 n = 0;
    auto start = attr_str.find("'");
    auto end = attr_str.rfind("'");
    ...
    return new Store30(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "transpose_13") {
    // does not have any attributes

    return new Transpose13(lhs_tensor, rhs_tensors);
  }
  else if (op_name == "mov_21") {
    // currently only implemented for one attribute: n
    int64 n = 0;
    auto start = attr_str.find("'");
    auto end = attr_str.rfind("'");
    ...
    return new Mov21(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "mov_23") {
    // currently only implemented for one attribute: n
    int64 n = 0;
    auto start = attr_str.find("'");
    auto end = attr_str.rfind("'");
    ...
    return new Mov23(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "gemm_33") {
    // does not have any attributes

    return new Gemm33(lhs_tensor, rhs_tensors);
  }
  else if (op_name == "gemm_13") {
    // does not have any attributes

    return new Gemm13(lhs_tensor, rhs_tensors);
  }
    ...
    return new Softmax(lhs_tensor, rhs_tensors, n);
  }
