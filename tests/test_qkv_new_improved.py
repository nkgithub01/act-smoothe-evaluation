...
class TestQKV:
    def test_backend_binary_exists(self):
        assert os.path.exists(os.path.join(WORK_DIR, "backends", "QKV_new_improved"))

    def test_compile_hlo(self):
        output_path = compile_hlo(WORK_DIR, QKV_DIR, "QKV_new_improved")
        assert os.path.exists(output_path)
        assert os.path.getsize(output_path) > 0

    def test_compiled_kernel_matches_golden(self):
        max_diff = run_compiled_kernel(WORK_DIR, os.path.join(QKV_DIR, "data"), "QKV_new_improved")
