...
create_instruction(const ParsedLine &pl,
  ...
  if (op_name == "slice") {
    ...
    return new LoadRm(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "load_cm") {
    // currently only implemented for one attribute: n
    int64 n = 0;
    auto start = attr_str.find("'");
    auto end = attr_str.rfind("'");
    ...
    return new LoadCm(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "store_rm") {
    // currently only implemented for one attribute: n
    int64 n = 0;
    auto start = attr_str.find("'");
    auto end = attr_str.rfind("'");
    ...
    return new StoreRm(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "store_cm") {
    // currently only implemented for one attribute: n
    int64 n = 0;
    auto start = attr_str.find("'");
    auto end = attr_str.rfind("'");
    ...
    return new StoreCm(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "mov") {
    // currently only implemented for one attribute: n
    int64 n = 0;
    ...
    return new Mov(lhs_tensor, rhs_tensors, n);
  }
  else if (op_name == "gemm") {
    // does not have any attributes

    return new Gemm(lhs_tensor, rhs_tensors);
  }
